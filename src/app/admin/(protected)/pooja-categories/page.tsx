...
import {
  PoojaCategory,
  useCreatePoojaCategory,
  useDeletePoojaCategory,
  usePoojaCategories,
  useUpdatePoojaCategory,
} from "@/features/admin/api/use-pooja-categories";
...
export default function PoojaCategoriesPage() {
  const { data: categories, isLoading } = usePoojaCategories();
  const createMutation = useCreatePoojaCategory();
  const updateMutation = useUpdatePoojaCategory();
  const deleteMutation = useDeletePoojaCategory();
  const [editing, setEditing] = useState<PoojaCategory | null>(null);
  ...
  const openEdit = (category: PoojaCategory) => {
    setEditing(category);
    reset({
      name: category.name,
      slug: category.slug,
  ...
  const onDelete = async (category: PoojaCategory) => {
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        description="Group poojas into browsable categories."
        actions={
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            <DialogFooter>
              <Button type="submit" disabled={isSubmitting}>
                ...
                {editing ? "Save changes" : `Create "${nameValue || "category"}"`}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
